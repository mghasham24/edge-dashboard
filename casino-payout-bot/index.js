...
`);
  ...
  // If the card is currently listed on the RS marketplace by its owner, we can't use it
  // for a payout offer — detect the warning and skip to the next card.
  const listedCheck = await safariEval(`
(function() {
  var w = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT);
  var n;
  while (n = w.nextNode()) {
    if (n.textContent.toLowerCase().includes('currently listed in the marketplace')) return 'listed';
  }
  return null;
})()
`);
  if (listedCheck === 'listed') {
    console.log('  ⚠  Card is listed on marketplace — skipping to next card.');
    ...
    while (cardTries < MAX_CARD_TRIES) {
      ...
      try { ok1 = await processOffer(p.cardUrl, p.offerAmount); }
        ...
        if (e.message && e.message.includes('Timeout')) {
          // Card page didn't respond as expected (likely listed/unavailable) — skip and retry
          await skipCard(entry.id);
          cardTries++;
          console.log(`  ↻ Card timed out (likely listed) — trying next card (${cardTries}/${MAX_CARD_TRIES})...`);
          await sleep(rand(800, 1200));
          continue;
        }
        ok1 = e; break; // non-timeout error — exit loop
      }

      if (ok1 === 'listed') {
        await skipCard(entry.id);
        cardTries++;
        console.log(`  ↻ Card listed — trying next card (${cardTries}/${MAX_CARD_TRIES})...`);
        await sleep(rand(800, 1200));
        continue;
      }
      break; // got a real result (true, false, or error)
    }
    if (noCard) continue;
    if (ok1 === 'listed') {
      console.error('  ✗ All candidate cards are listed — will retry next run.');
      await markAttempt(entry.id, 'all candidate cards listed on marketplace');
      continue;
    }
