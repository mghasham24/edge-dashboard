    ...
    function otdToggleOverlaps() {
        ...
        if (!otdShowOverlaps) { otdAllOverlapsView = false; otdOverlapSportDropdownOpen = false; }
        renderOtdResults();
    }
    ...
    function otdToggleOverlapSport(sport) {
        if (!otdOverlapSportFilter) otdOverlapSportFilter = {};
        if (otdOverlapSportFilter[sport]) {
            delete otdOverlapSportFilter[sport];
            if (Object.keys(otdOverlapSportFilter).length === 0) otdOverlapSportFilter = null;
        } else {
    ...
    function otdToggleOverlapSportDropdown() {
        otdOverlapSportDropdownOpen = !otdOverlapSportDropdownOpen;
        otdOverlapPanelCache = null;
        renderOtdResults();
    }
    ...
    function renderOtdCheckResults() {
        ...
        otdCheckEarnings.forEach(function(e) {
            ...
            if (existingEntries.length > 0) {
                ...
                // Cross-sport 3rd slot: when limit=3, only ONE 3rd claim is allowed across all sports.
                // The sport with the highest 3rd-position earner wins the shared slot.
                if (limit === 3 && !isOver && newRax >= 200) {
                    var higherSameSport = existingEntries.filter(function(x) { return (x.rax || 0) > newRax; }).length;
                    if (higherSameSport === 2) { // new card would rank exactly 3rd in its sport
                        var allDayEntries = checkDateMap[dayKey] || [];
                        var otherSportGroups = {};
                        allDayEntries.forEach(function(entry) {
                            var ns = normSport(entry.player.sport);
                            if (ns === normCheckSport) return;
                            if (!otherSportGroups[ns]) otherSportGroups[ns] = [];
                            otherSportGroups[ns].push(entry);
                        });
                        Object.keys(otherSportGroups).forEach(function(ns) {
                            var sorted = otherSportGroups[ns].slice().sort(function(a, b) { return (b.rax || 0) - (a.rax || 0); });
                            var third = sorted[2]; // 3rd-highest earner in that sport
                            if (third && (third.rax || 0) > 199 && (third.rax || 0) > newRax) {
                                crossSportBlockers.push(third);
                            }
                        });
                        if (crossSportBlockers.length > 0) {
                            isOver = true;
                            wasted = true;
                            dayWastedRax = newRax;
                        }
                ...
                if (isOver) overlapDays.push({ day: dayKey, entries: existingEntries, total: newTotal, wasted: wasted, newRax: newRax, wastedRax: dayWastedRax, crossSportBlockers: crossSportBlockers });
            }
        });
        ...
        if (realConflicts.length > 0) {
            ...
            realConflicts.forEach(function(d) {
                ...
                // Cross-sport conflict: the 3rd position is wasted even though same-sport limit isn't exceeded
                var crossBlockers = d.crossSportBlockers || [];
                var effectiveLimit = crossBlockers.length > 0 ? limit - 1 : limit;
                var cardRows = allCards.map(function(c, idx) {
                    var claimed = idx < effectiveLimit;
                ...
                var crossBlockHtml = '';
                if (crossBlockers.length > 0) {
                    crossBlockHtml = '<div style="font-size:10px;color:var(--muted2);margin-top:6px;border-top:1px solid var(--border2);padding-top:4px">' +
                        '3rd slot taken by: ' +
                        crossBlockers.map(function(b) {
                            return '<span style="color:#ef5350;font-weight:600">' + escHtml(b.player.name) + '</span>' +
                                ' · ' + escHtml((b.player.sport || '').toUpperCase()) +
                                ' · ' + RAX_ICON + (b.rax || 0).toLocaleString();
                        }).join(', ') +
                    '</div>';
                }
    ...
    function parlayRenderSlip() {
        ...
        var badge = document.getElementById('parlay-pick-badge');
        if (badge) badge.textContent = count;

        var ready = count >= 2;
        var legsRow = document.getElementById('parlay-legs-row');
        if (legsRow) {
            legsRow.innerHTML = '<span class="parlay-legs-label">Legs</span>' +
                [1,2,3,4,5].map(function(i) {
                    return '<div class="parlay-legs-dot' + (i <= count ? (ready ? ' ready' : ' on') : '') + '"></div>';
                }).join('');
        }
        ...
        if (payout !== null && payEl) {
            ...
            if (noteEl) noteEl.textContent = 'Select 2–5 players to continue';
        }
    }
    ...
    function renderOtdPanel() {
        ...
        var loadedChip = (otdMode === 'username' && otdSelectedUser && otdPlayers.length > 0 && !otdLoadingPasses)
            ? '<span style="display:inline-flex;align-items:center;gap:5px;font-size:11px;font-weight:700;color:var(--accent);background:rgba(99,102,241,.12);border:1px solid rgba(99,102,241,.3);border-radius:20px;padding:3px 10px;white-space:nowrap">✓ ' + escHtml(otdSelectedUser.username) + '</span>'
            : '';

        var tabBase = 'border:none;font-family:var(--sans);font-size:12px;font-weight:700;padding:6px 16px;border-radius:5px;cursor:pointer';
        var tabActive = tabBase + ';background:var(--accent);color:#fff';
        var tabInactive = tabBase + ';background:transparent;color:var(--muted)';
        ...
        if (otdMode === 'suggest') {
            ...
            inputSection =
                '<div style="display:flex;flex-wrap:wrap;gap:8px;align-items:flex-end;margin-bottom:10px">' +
                    '<div style="position:relative;flex:1;min-width:160px">' +
                        '<input id="otd-search-input" type="text" placeholder="Search player or team…" autocomplete="off" ' +
                            'style="width:100%;box-sizing:border-box;background:var(--bg3);border:1px solid var(--border2);color:var(--fg);font-family:var(--sans);font-size:13px;padding:8px 10px;border-radius:6px" ' +
                            'oninput="otdOnSearchInput(this.value)" />' +
                        '<div id="otd-autocomplete" style="display:none;position:absolute;top:100%;left:0;right:0;background:var(--bg2);border:1px solid var(--border2);border-radius:6px;z-index:200;margin-top:3px;overflow:hidden;max-height:220px;overflow-y:auto"></div>' +
                    '</div>' +
                    '<select id="otd-sport-sel" onchange="otdUpdateSeasonOpts(this.value)" ' +
                        'style="background:var(--bg3);border:1px solid var(--border2);color:var(--fg);font-family:var(--sans);font-size:12px;padding:8px 6px;border-radius:6px;cursor:pointer">' +
                        sportOpts + '</select>' +
                    '<select id="otd-season-sel" ' +
                        'style="background:var(--bg3);border:1px solid var(--border2);color:var(--fg);font-family:var(--sans);font-size:12px;padding:8px 6px;border-radius:6px;cursor:pointer">' +
                        otdSeasonOpts(defaultSport) + '</select>' +
                    '<select id="otd-level-sel" ' +
                        'style="background:var(--bg3);border:1px solid var(--border2);color:var(--fg);font-family:var(--sans);font-size:12px;padding:8px 6px;border-radius:6px;cursor:pointer">' +
                        levelOpts + '</select>' +
                    ...
                    (otdPlayers.length > 0 ? '<button onclick="otdClearAll()" style="background:var(--bg3);border:1px solid var(--border2);color:var(--muted);font-family:var(--sans);font-size:12px;padding:8px 12px;border-radius:6px;cursor:pointer">Clear All</button>' : '') +
                '</div>';
        }
    ...
    function renderOtdLeaderboard() {
        ...
        var rowData = filtered.map(function(p, i) {
            ...
            var yearHtml = p.season && p.season !== 'alltime' ? ' <span style="font-size:' + (mob ? '8px' : '11px') + ';font-weight:400;color:var(--muted);opacity:.6">' + escHtml(otdFormatSeason(rowSport, p.season)) + '</span>' : '';
            var posHtml = '';
        ...
        otdSearchTimer = setTimeout(function() {
            ...
            fetch('/api/real/otd?action=search&q=' + encodeURIComponent(val) + '&sport=' + sport, { credentials: 'same-origin' })
                ...
                .then(function(d) {
                    ...
                    d.players.forEach(function(p) {
                        var row = document.createElement('div');
                        row.style.cssText = 'padding:8px 12px;cursor:pointer;font-size:13px;border-bottom:1px solid var(--border);display:flex;align-items:center;justify-content:space-between;gap:8px';
                        var label = document.createElement('span');
                        label.textContent = p.name;
                        var badge = document.createElement('span');
                        badge.style.cssText = 'font-size:10px;font-weight:700;color:var(--muted);background:var(--bg3);padding:2px 6px;border-radius:4px;white-space:nowrap;flex-shrink:0';
                        badge.textContent = p.sport.toUpperCase() + (p.entityType === 'team' ? ' · Team' : '');
    ...
    function otdAddPlayer() {
        ...
        var lbl    = (OTD_LEVEL_OPTIONS.find(function(o) { return o.value === level; }) || {}).label || 'Level ' + level;
        var et     = otdSelectedPlayer.entityType || 'player';
        ...
        otdColorIdx++;
        var entry = {
            id: otdSelectedPlayer.id, name: otdSelectedPlayer.name,
            sport: sport, season: season, level: level, levelLabel: lbl, color: color,
            earnings: null, baseTotal: null, entityType: et,
            avatar: otdSelectedPlayer.avatar || '', rarityColor: otdRarityColor(level),
            backgroundSource: null, _origLevel: level, _origMultiplier: null
        };
        otdPlayers.push(entry);
        ...
        fetch('/api/real/otd?action=earnings&id=' + encodeURIComponent(entry.id) + '&sport=' + encodeURIComponent(sport) + '&season=' + encodeURIComponent(season) + '&level=' + level + '&entityType=' + encodeURIComponent(et), { credentials: 'same-origin' })
            ...
            .then(function(d) {
                ...
                if (loadedCount === 1 && entry.earnings.length) {
                    var curYr = new Date().getFullYear();
                    var todayStr = new Date().toISOString().slice(0, 10);
                    var dates = entry.earnings.map(function(e) {
                        var dp2 = (e.day || '').split('T')[0].split('-');
                        return dp2.length === 3 ? (curYr + '-' + dp2[1].padStart(2,'0') + '-' + dp2[2].padStart(2,'0')) : '';
                    }).filter(Boolean).sort();
                    var target = dates.filter(function(x) { return x >= todayStr; })[0] || dates[dates.length - 1];
                    if (target) {
                        var tp = target.split('-');
    ...
    function renderOtdResults() {
        ...
        if (otdAllOverlapsView) {
            ...
            var aoSelectedCount = otdOverlapSportFilter ? Object.keys(otdOverlapSportFilter).length : 0;
            var aoSportsBtnLabel = 'Sports' + (aoSelectedCount > 0 ? ' (' + aoSelectedCount + ')' : '') + ' ▾';
            var aoSportsBtn = aoSports.length > 1
                ? '<button onclick="otdToggleOverlapSportDropdown()" style="font-family:var(--sans);font-size:11px;font-weight:600;padding:3px 10px;border-radius:4px;cursor:pointer;border:1px solid ' + (aoSelectedCount > 0 ? 'var(--accent);background:rgba(99,102,241,.15);color:var(--accent)' : 'var(--border2);background:var(--bg3);color:var(--muted)') + '">' + aoSportsBtnLabel + '</button>'
                : '';
            var aoSportsDropdown = '';
            if (otdOverlapSportDropdownOpen && aoSports.length > 1) {
                aoSportsDropdown = '<div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:10px">' +
                    aoSports.map(function(sp) {
                        var checked = otdOverlapSportFilter && otdOverlapSportFilter[sp];
                        var label = (OTD_SPORTS_LIST.find(function(s) { return s.key === sp; }) || {}).label || sp.toUpperCase();
                        return '<button onclick="otdToggleOverlapSport(\'' + sp + '\')" style="font-family:var(--sans);font-size:11px;font-weight:600;padding:3px 10px;border-radius:4px;cursor:pointer;border:1px solid ' + (checked ? 'var(--accent);background:rgba(99,102,241,.15);color:var(--accent)' : 'var(--border2);background:var(--bg3);color:var(--muted)') + '">' + (checked ? '✓ ' : '') + escHtml(label) + '</button>';
                    }).join('') +
                '</div>';
            }

            var aoFiltered = otdOverlapSportFilter
                ? aoFlat.filter(function(e) { return otdOverlapSportFilter[e.w.player.sport]; })
                : aoFlat;
            ...
            el.innerHTML =
                '<div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:12px;gap:8px">' +
                    ...
                    '<div style="text-align:right;flex-shrink:0">' +
                        '<div style="font-family:var(--mono);font-size:13px;font-weight:700;color:#ef5350">' + RAX_ICON + aoTotalRax.toLocaleString() + ' wasted</div>' +
                    '</div>' +
                '</div>' +
                aoSportsDropdown +
                '<div style="font-size:11px;color:var(--muted2);margin-bottom:8px">' + aoFiltered.length + ' overlap' + (aoFiltered.length !== 1 ? 's' : '') + (otdOverlapSportFilter ? ' in ' + Object.keys(otdOverlapSportFilter).length + ' sport' + (Object.keys(otdOverlapSportFilter).length > 1 ? 's' : '') : ' across all sports') + ' — tap ' + AO_NAV_ICON + ' to jump to that day on the calendar</div>' +
        ...
        var overlapKeys = Object.keys(otdOverlapMap).sort(function(a, b) {
            if (otdOverlapSort === 'date-asc') return a < b ? -1 : a > b ? 1 : 0;
            if (otdOverlapSort === 'date-desc') return a > b ? -1 : a < b ? 1 : 0;
            return a < b ? -1 : a > b ? 1 : 0; // date-asc base for rax sorts (flattened below)
        });
        // Flat list of all wasted entries — used for rax sorts so individual rows are ordered by rax
        var overlapFlat = [];
        overlapKeys.forEach(function(dk) {
            otdOverlapMap[dk].forEach(function(g) {
                g.wasted.forEach(function(w) { overlapFlat.push({ dk: dk, g: g, w: w }); });
            });
        });
        if (otdOverlapSort === 'rax-desc') overlapFlat.sort(function(a, b) { return (b.w.rax||0) - (a.w.rax||0); });
        if (otdOverlapSort === 'rax-asc')  overlapFlat.sort(function(a, b) { return (a.w.rax||0) - (b.w.rax||0); });
        ...
        if (otdShowOverlaps && overlapCount > 0) {
            ...
            var sportFilterKey = otdOverlapSportFilter ? Object.keys(otdOverlapSportFilter).sort().join(',') : '';
            var ovlCacheKey = (otdOverlapSort || '') + '|' + otdOverlapPage + '|' + sportFilterKey + '|' + (otdOverlapSportDropdownOpen ? '1' : '0');
            if (otdOverlapPanelCache && otdOverlapPanelCache.key === ovlCacheKey) {
                ...
                var ovlSelectedCount = otdOverlapSportFilter ? Object.keys(otdOverlapSportFilter).length : 0;
                var ovlSportsBtnLabel = 'Sports' + (ovlSelectedCount > 0 ? ' (' + ovlSelectedCount + ')' : '') + ' ▾';
                var ovlSportsBtn = ovlSports.length > 1
                    ? '<button onclick="otdToggleOverlapSportDropdown()" style="' + ovlSortBase + (ovlSelectedCount > 0 ? 'var(--accent);background:rgba(99,102,241,.15);color:var(--accent)' : 'var(--border2);background:var(--bg3);color:var(--muted)') + '">' + ovlSportsBtnLabel + '</button>'
                    : '';
                var ovlSportsDropdown = '';
                if (otdOverlapSportDropdownOpen && ovlSports.length > 1) {
                    ovlSportsDropdown = '<div style="display:flex;align-items:center;gap:4px;padding:6px 10px;border-bottom:1px solid var(--border);overflow-x:auto;white-space:nowrap;flex-wrap:wrap">' +
                        ovlSports.map(function(sp) {
                            var checked = otdOverlapSportFilter && otdOverlapSportFilter[sp];
                            var label = (OTD_SPORTS_LIST.find(function(s) { return s.key === sp; }) || {}).label || sp.toUpperCase();
                            return '<button onclick="otdToggleOverlapSport(\'' + sp + '\')" style="font-family:var(--sans);font-size:10px;font-weight:600;padding:2px 8px;border-radius:4px;cursor:pointer;border:1px solid ' + (checked ? 'var(--accent);background:rgba(99,102,241,.15);color:var(--accent)' : 'var(--border2);background:var(--bg3);color:var(--muted)') + '">' + (checked ? '✓ ' : '') + escHtml(label) + '</button>';
                        }).join('') +
                    '</div>';
                }
                // Apply sport filter then paginate
                var filteredFlat = otdOverlapSportFilter
                    ? overlapFlat.filter(function(e) { return otdOverlapSportFilter[e.w.player.sport]; })
                    : overlapFlat;
                ...
                overlapPanel = '<div style="background:var(--bg2);border:1px solid var(--border2);border-radius:8px;margin-bottom:10px;overflow:hidden">' +
                    '<div style="display:flex;align-items:flex-start;justify-content:space-between;padding:8px 10px;border-bottom:1px solid var(--border);gap:6px">' +
                        ...
                        '</div>' +
                        '<div style="text-align:right;flex-shrink:0">' +
                            '<div style="font-family:var(--mono);font-size:12px;font-weight:700;color:#ef5350;margin-bottom:4px">' + RAX_ICON + overlapTotal.toLocaleString() + '</div>' +
                            '<button onclick="otdShowAllOverlapsView()" style="font-family:var(--sans);font-size:9px;font-weight:600;padding:1px 6px;border-radius:4px;cursor:pointer;border:1px solid var(--border2);background:var(--bg3);color:var(--muted)">Show All →</button>' +
                        '</div>' +
                    '</div>' +
                    ovlSportsDropdown +
