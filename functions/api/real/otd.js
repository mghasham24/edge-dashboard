...
export async function onRequestGet(context) {
  ...
  // Admin debug: raw leaderboard response — shows field names so we can parse level/id correctly
  if (action === 'discover_raw') {
    if (!session.is_admin) return fail(403, 'Admin only');
    const sport = url.searchParams.get('sport') || 'mlb';
    const season = url.searchParams.get('season') || String(new Date().getFullYear());
    const entity = url.searchParams.get('entity') || 'player';
    const before = url.searchParams.get('before') || '0';
    const lbUrl = `${RS_BASE}/userpassshop/${sport}/season/${season}/entity/${entity}/section/earningstotal?before=${before}`;
    const res = await fetch(lbUrl, { headers });
    const text = await res.text();
    let parsed; try { parsed = JSON.parse(text); } catch(e) { parsed = text; }
    return new Response(JSON.stringify({ status: res.status, url: lbUrl.replace(RS_BASE,''), parsed }, null, 2), { headers: { 'Content-Type': 'application/json' } });
  }

  // Admin: paginate leaderboard, batch-check D1, return uncached {id,level,sport,season,entityType} list
  if (action === 'discover_all') {
    if (!session.is_admin) return fail(403, 'Admin only');
    const sport      = url.searchParams.get('sport') || 'mlb';
    const season     = url.searchParams.get('season') || String(new Date().getFullYear());
    const entity     = url.searchParams.get('entity') || 'player';
    const maxPages   = parseInt(url.searchParams.get('max_pages') || '20', 10);
    ...
    while (pages < maxPages) {
      ...
      let data;
      try {
        const res = await fetch(lbUrl, { headers, signal: AbortSignal.timeout(8000) });
        if (!res.ok) break;
        data = await res.json();
      } catch(e) { break; }

      const items = data.items || data.passes || data.data || data.collectingCards || [];
      if (!items.length) break;

      for (const item of items) {
        const entityId = item.entityId || item.playerId || item.teamId || (item.entity||{}).id || (item.player||{}).id || (item.team||{}).id;
        if (!entityId) continue;

        // Try to extract level from response; expand to common levels if unknown
        const rawRarity = (item.rarity || item.rarityName || '').toLowerCase();
        const rawLevel = typeof item.level === 'number' ? item.level
          : (item.boostInfo && typeof item.boostInfo.level === 'number') ? item.boostInfo.level
          : rawRarity === 'rare' ? 3 : rawRarity === 'epic' ? 4
          : rawRarity === 'legendary' ? 5 : 0;

        if (rawLevel > 0) {
          allItems.push({ id: entityId, level: rawLevel, entityType: entity, sport, season });
        } else {
          // Level unknown — try rare(3), epic(4), legendary(5), legendary+1(6)
          for (const lvl of [3, 4, 5, 6]) {
            allItems.push({ id: entityId, level: lvl, entityType: entity, sport, season });
          }
        }
      }

      // Pagination: look for nextBefore / cursor in response
      const next = data.nextBefore ?? data.cursor ?? data.before ?? data.nextPage ?? data.offset ?? null;
      if (!next || next === before || String(next) === '0') break;
      before = String(next);
      pages++;
      // Small pause between leaderboard pages to avoid RS rate limit
      if (pages < maxPages) await new Promise(r => setTimeout(r, 300));
    }

    if (!allItems.length) {
      return new Response(JSON.stringify({ ok: true, total: 0, uncached: [], pages }), { headers: { 'Content-Type': 'application/json' } });
    }

    // Batch-check D1: split into chunks of 50 to respect D1 bind limit
    const uncached = [];
    const chunkSize = 50;
    for (let i = 0; i < allItems.length; i += chunkSize) {
      const chunk = allItems.slice(i, i + chunkSize);
      const cacheKeys = chunk.map(c => `otd_earnings_v9_${c.entityType}_${c.sport}_${c.season}_${c.id}_l${c.level}`);
      try {
        const placeholders = cacheKeys.map(() => '?').join(',');
        const existing = await env.DB.prepare(`SELECT cache_key FROM odds_cache WHERE cache_key IN (${placeholders})`).bind(...cacheKeys).all();
        const existingSet = new Set((existing.results || []).map(r => r.cache_key));
        chunk.forEach((c, idx) => { if (!existingSet.has(cacheKeys[idx])) uncached.push(c); });
      } catch(e) {
        chunk.forEach(c => uncached.push(c));
      }
    }

    return new Response(JSON.stringify({ ok: true, total: allItems.length, uncached_count: uncached.length, uncached, pages }), { headers: { 'Content-Type': 'application/json' } });
  }
