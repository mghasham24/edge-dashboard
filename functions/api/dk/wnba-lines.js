// functions/api/dk/wnba-lines.js
// GET /api/dk/wnba-lines
// Returns today's WNBA game lines (ML, Spread, Total) from DK for the parlay builder.

import { getSessionOrCron } from '../../_lib/auth.js';

const DK_BASE       = 'https://sportsbook-nash.draftkings.com/sites/US-SB/api/sportscontent';
const DK_LEAGUE     = '94682'; // WNBA
const LINES_SUBCAT  = '4511';  // WNBA game lines (ML, spread, total) — from DevTools
const EVENTS_SUBCAT = '12488'; // Points O/U — used only to discover today's event IDs
const CACHE_TTL     = 60;
const CACHE_KEY     = 'dk_wnba_lines_v1';
...
function eventsUrl() {
  const eq = encodeURIComponent(`$filter=leagueId eq '${DK_LEAGUE}' AND clientMetadata/Subcategories/any(s: s/Id eq '${EVENTS_SUBCAT}')`);
  const mq = encodeURIComponent(`$filter=clientMetadata/subCategoryId eq '${EVENTS_SUBCAT}' AND tags/all(t: t ne 'SportcastBetBuilder')`);
  return `${DK_BASE}/controldata/league/leagueSubcategory/v1/markets?isBatchable=false&templateVars=${DK_LEAGUE}%2C${EVENTS_SUBCAT}&eventsQuery=${eq}&marketsQuery=${mq}&include=Events&entity=events`;
}
...
function parseLines(data, eventId, homeTeam, awayTeam, homeShort, awayShort, timeStr, startMs) {
  ...
  for (const mkt of (data.markets || [])) {
    ...
    if (name.includes('moneyline') || name.includes('money line')) {
    ...
    } else if (name.includes('point spread') || name.includes('spread')) {
...
export async function onRequestGet(context) {
  ...
  // Step 1: discover today's events from Points O/U subcategory
  const evRes = await fetch(eventsUrl(), { headers: DK_HEADERS, signal: AbortSignal.timeout(12000) });
  if (!evRes.ok) return new Response(JSON.stringify({ ok: false, error: 'events fetch failed' }), { headers: { 'Content-Type': 'application/json' } });
