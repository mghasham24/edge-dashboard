...
export async function onRequestPost({ request, env }) {
  try {
    ...
    // Link RS account to this RaxEdge user — UPDATE existing row first,
    // INSERT with empty auth_token placeholder only if no row exists yet
    const upd = await env.DB.prepare(
      'UPDATE real_auth SET rs_user_id=?, rs_username=?, parlay_verified=1, parlay_verified_at=?, updated_at=? WHERE user_id=?'
    ).bind(codeRow.rs_user_id, codeRow.rs_username, now, now, session.user_id).run();
    if ((upd.meta?.changes ?? upd.changes ?? 0) === 0) {
      await env.DB.prepare(
        'INSERT INTO real_auth (user_id, auth_token, rs_user_id, rs_username, parlay_verified, parlay_verified_at, updated_at) VALUES (?,?,?,?,1,?,?)'
