...
async function placeLegsAndRespond(db, parlayId, cardId, legs, stake, payoutRax, expiresAt, rsUsername, now, isFreePlay = false) {
  await db.batch(legs.map(leg =>
    db.prepare(
      ...
      'american_odds, implied_prob, headshot_url, game_start_ms, rs_game_id, team) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
    ).bind(
      parlayId, leg.sport,
      ...
      leg.rsGameId || null,
      typeof leg.team === 'string' ? leg.team.slice(0, 12) : null
    )
  ));
