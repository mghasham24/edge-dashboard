#!/usr/bin/env node
// card-buyer/casino-buyer.js — AppleScript edition
// Buys NBA 2026 10 Rax RS marketplace cards for the casino deposit pool.
...
//
// Usage:
//   node casino-buyer.js        ← buy NBA 2026 cards (casino pool)
//   node casino-buyer.js --test ← dry run (finds cards, skips Place Bid)
...
async function selectSport() {
  console.log(`  Selecting ${SPORT} in marketplace at screen (189, 205)...`);
  await systemClick(189, 205);
  await sleep(2500);
}

async function selectYear() {
  // Read currently displayed year in the filter bar (below sport tabs, above card list)
...
(function() {
  ...
  while (n = w.nextNode()) {
    ...
    if (/^20\\d\\d(-\\d\\d)?$/.test(t)) {
      var r = n.parentElement.getBoundingClientRect();
      if (r.width > 0 && r.top > 80 && r.top < 300) return t;
    }
  }
  return null;
})()
`);
  ...
  if (currentYear === YEAR) {
    console.log(`  Year already ${YEAR}.`);
    return;
  }

  console.log(`  Changing year from ${currentYear || '?'} to ${YEAR}...`);

  // Click the year element to open the dropdown — wider Y range, longer timeout
  const yearClicked = await safariEval(`
(function() {
  var w = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT);
  var n;
  while (n = w.nextNode()) {
    ...
    if (/^20\\d\\d$/.test(t)) {
      var r = n.parentElement.getBoundingClientRect();
      if (r.width > 0 && r.top > 50 && r.top < 500) {
        n.parentElement.click();
        return 'clicked-at-' + Math.round(r.top);
      }
    }
  }
  return null;
...
`);
  if (!yearClicked) {
    console.log(`  Year element not found — may already be ${YEAR}, continuing.`);
    return;
  }
  console.log(`  Year dropdown opened (${yearClicked})`);
  await sleep(700);

  // Click our target year in the dropdown
...
async function main() {
  console.log(`\n Card Buyer  |  ${SPORT} ${YEAR}  |  ${DRY_RUN ? 'DRY RUN' : 'LIVE'}`);
